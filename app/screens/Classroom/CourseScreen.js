...
function editCourse(Id,userId,course,CourseNameID,title,instruction,document){
  navigation.navigate('EditCourseContent',{ID:Id,UserID:userId,Faculty:Faculty,Department:Department,Level:Level,Year:Year,Course : course,CourseID : CourseID,CourseNameID:CourseNameID,title : title,instruction:instruction,document:document});
}

function Edit(Id,userId,course,CourseNameID,title,instruction,document) {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: "Edit", onPress: () => editCourse(Id,userId,course,CourseNameID,title,instruction,document) },
    ...
    return(
      <View style={styles.container}>
        <View style={{ backgroundColor: "white", height: hp("10%") }}>
          ...
          >
            <Text style = {{fontSize:hp('2.3%'),fontWeight:'500' ,alignSelf:'center'}}>{CourseName}</Text>
        ...
        <View>
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              >
                <Text  style={{paddingBottom:20 ,fontSize:hp('2%'),fontWeight:'bold'}}><Entypo name="book" size={hp('2%')} color="black" /> {item.title}</Text>
                <Text style={{paddingLeft:10 ,paddingBottom:10}}>{item.instruction}</Text>
                <Text style={{paddingLeft:10 ,fontSize:hp('1.5%'),paddingTop:20}}>Document Link</Text>
                <TouchableOpacity  onPress={()=>Linking.openURL(item.document)}>
                 <Text style = {{paddingLeft:10 ,paddingTop:10,fontSize:hp('2%'),color:'red'}}><FontAwesome5 name="link" size={hp('1.5%')} color="red" /> Link</Text>
                 </TouchableOpacity>
  
                 <TouchableOpacity  style= {{marginRight:wp('8%'), marginTop:hp('1%'), alignSelf:'flex-end'}}
                        onPress={() => Edit( item.Id,item.userId,item.course,item.CourseNameID,item.title,item.instruction,item.document)}
                      >
                        <AntDesign name="edit" size={hp('3%')} color="#cdaffa" />
                        <Text style={{ fontSize: hp("1.4%") }}>Edit</Text>
                      </TouchableOpacity>
                <Text style ={{alignSelf:'flex-start',fontSize:hp('1%'),marginLeft:wp('3%')}}>{item.dateAndTime}</Text>
